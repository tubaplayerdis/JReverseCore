...
#include "JReverseCommandParser.h"
        ...
        if (strcmp(JReverseStore::ruleslist[i].ClassName, name)) {
            if (!JReverseStore::ruleslist[i].IsBypass) {
                ...
                *new_class_data = new unsigned char[result_size];
                ...
                if (JReverseStartupSettings::isClassFileLoadMessages) std::cout << "\033[34mModified class by rule: \033[0m" << name << std::endl;
                ClassFile curfile;
...
void MainThread(HMODULE instance)
{
    ...
    */
    ...
    std::cout << R"(     | |  _ \ _____   _____ _ __ ___  ___  \ \   / / | / _ \ )" << std::endl;
    std::cout << R"(  _  | | |_) / _ \ \ / / _ \ '__/ __|/ _ \  \ \ / /| || | | |)" << std::endl;
